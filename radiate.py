...
class Sequence:
    ...
    def __init__(self, sequence_path, config_file='config/config.yaml'):
        ...
        with open(self.config['calib_file'], 'r') as file:
            self.calib = yaml.safe_load(file)
        ...
        if (get_annotations):
            ...
            if (self.annotations != None):
                ...
                if self.config['use_camera_right_rect']:
                    ...
                    bboxes_3d = self.project_bboxes_to_camera(radar_annotations,
                                                              self.calib.right_cam_mat,
                                                              self.calib.RadarToRight)
                    annotations['camera_right_rect'] = self.Transform_3D_2D(bboxes_3d, output['sensors']['camera_right_rect'].shape, pc_size1 = 0.78, pc_size2 = 0.9)
    ...
    def vis_bbox_cam(self, image, bboxes_3d, pc_size1, pc_size2):
        ...
        for obj in bboxes_3d:
            ...
            if obj['bbox_3d'].shape[0] > 0:
                ...
                wid = bb[2] - bb[0]
                hei = bb[3] - bb[1]
                bb[0] += wid*(1.0 - pc_size1-0.1)
                bb[2] -= wid*(1.0 - pc_size1)
                bb[1] += hei*(1.0 - pc_size2)
                bb[3] -= hei*(1.0 - pc_size2)
                bb = bb.astype(np.int)
                vis_im = cv2.rectangle(
                    vis_im, (bb[0], bb[1]), (bb[2], bb[3]), (np.array(color) * 255).astype(np.int).tolist())

        return vis_im
    def Transform_3D_2D(self, bboxes_3d, shape, pc_size1, pc_size2):
        """diplay pseudo 2d bounding box from camera
        :param image: camera which the bounding box is going to be projected
        :type image: np.array
        :param bboxes_3d: list of bounding box information with pseudo-3d image coordinate frame
