...
#fuction to save the weights of the most performed model 
class SaveBestModel:
    """
...
for epoch in range(50): #I decided to train the model for 50 epochs
    ...
    for batch_idx, (image_camera_right, image_camera_left, lidar_bev_image, radar_cart, targets) in enumerate(train_loader):
        ...
        #print(scores)
        loss = criterion(scores,targets[0])
        loss.backward()
        optimizer.step()
        loss_ep += loss.item()
    ...
    with torch.no_grad():
        num_correct = 0
        num_samples = 0
        for batch_idx, (image_camera_right, image_camera_left, lidar_bev_image, radar_cart, targets) in enumerate(valid_loader):
            print(batch_idx)
            image_camera_right = image_camera_right.to(device=device)
            image_camera_left = image_camera_left.to(device=device)
            lidar_bev_image = lidar_bev_image.to(device=device)
        ...
        save_best_model(
        num_correct, epoch, model, optimizer
    )
